package com.pomelo.ddd;

import com.pomelo.ddd.entity.AggregateEntity;
import com.pomelo.ddd.manager.AggregateManager;
import com.pomelo.demo.AttendYuWenKe;
...
public class Pomelo<T> {
    ...
    public <R, C> R command(C c) {
        try {
            return (R) aggregateEntity.getCommandHandlerMap().get(AttendYuWenKe.class).invoke(t, c);
        } catch (IllegalAccessException e) {
            e.printStackTrace();
        } catch (InvocationTargetException e) {
            e.printStackTrace();
